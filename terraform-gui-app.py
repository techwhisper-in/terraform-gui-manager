...
class CustomDialog:
    def __init__(self, parent, title, message, approve_text="Approve"):
        ...
        # Message
        msg_label = ttk.Label(self.top, text=message, wraplength=380, justify='left')
        msg_label.pack(pady=15, padx=20, fill=tk.BOTH, expand=True)

        # Button frame
        btn_frame = ttk.Frame(self.top)
        btn_frame.pack(pady=10)
        
        ttk.Button(btn_frame, text=approve_text, command=self.approve).pack(side=tk.LEFT, padx=10)
        ttk.Button(btn_frame, text="Reject", command=self.reject).pack(side=tk.LEFT, padx=10)
        if "DESTRUCTIVE" in title:
            ttk.Style().configure("Destructive.TButton", foreground="black", background="#d9534f")
            self.top.configure(background="#f2dede")
            btn_frame.children['!button'].configure(style="Destructive.TButton")
        self.top.protocol("WM_DELETE_WINDOW", self.reject)
...
class TerraformGUI:
    ...
    def run_apply(self):
        dialog = CustomDialog(
            self.root,
            "Confirm Apply",
            "You are about to apply Terraform configuration.\n"
            "This will create/modify infrastructure resources.",
            approve_text="Apply Changes"
        )
        if dialog.result:
    ...
    def run_destroy(self):
        dialog = CustomDialog(
            ...
            "This action cannot be undone. Ensure you have proper backups.",
            approve_text="Confirm Destruction"
        )
        if dialog.result:
